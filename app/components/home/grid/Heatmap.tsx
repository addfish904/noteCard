...
export default function Heatmap() {
  ...
  return (
    <Card className="flex justify-between items-center px-4 py-4 gap-7 sm:flex-row flex-col">
      <div className="flex flex-col items-start justify-center text-left h-full w-full rounded-2xl border border-gray-300 bg-gray-50 p-6 dark:bg-[#292929]">
        ...
        <div className="mt-2">
          <p>
            連續記錄{" "}
            <span className="font-semibold text-4xl">{streakDays}</span> 天 /
          </p>
        </div>
        <div>
          <p>
            一共累積{" "}
            <span className="font-semibold text-4xl">{thisMonthTotal}</span>{" "}
            篇筆記
          </p>
          {percentChange !== null && (
            <p
          ...
          <div className="flex justify-center text-xs text-gray-500">
            {weeks.map((_, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="w-4 h-4 m-[1px] flex justify-center text-[10px]"
                >
                  {label ? label.month : ""}
                </div>
              );
